...
import { resolveUcFinalUrl } from './parsers/uc'
 ...
 * UC 网盘下载代理。
 *
 * UC CDN 直链的签名与"签发签名时的请求方 IP"绑定，签发和下载必须是同一个来源 IP，
 * 否则会被判定签名不符、返回 403（跟夸克/百度网盘同一个限制）。
 * 因此把"调用 file/download 现场签发直链"这一步推迟到访客真正点击下载的这一次请求内完成，
...
export const onRequestGet: PagesFunction<Env> = async ({ request, env }) => {
  ...
  let cookie: string
  try {
    const resolved = await resolveUcFinalUrl(env, token)
    finalUrl = resolved.url
    cookie = resolved.cookie
  } catch (error) {
    const message = error instanceof Error ? error.message : 'UC 网盘解析失败'
  ...
  if (!upstreamRes.ok && upstreamRes.status !== 206) {
    return new Response(`UC 网盘下载失败（上游状态码 ${upstreamRes.status}）`, { status: 502 })
  }
